...
const GALLERY_ITEMS: GalleryItem[] = [
  { id: 1, title: 'DARK DASHBOARD',     tag: 'UI/UX',      image: 'https://picsum.photos/seed/kai1/400/300' },
  { id: 2, title: 'CYBER DAWN',         tag: 'ART',        image: 'https://picsum.photos/seed/kai2/400/300' },
  { id: 3, title: 'PORTAL INTERFACE',   tag: 'WEB',        image: 'https://picsum.photos/seed/kai3/400/300' },
  { id: 4, title: 'VOID LAB',           tag: 'EXPERIMENT', image: 'https://picsum.photos/seed/kai4/400/300' },
  { id: 5, title: 'NEON GRID',          tag: 'ART',        image: 'https://picsum.photos/seed/kai5/400/300' },
  { id: 6, title: 'PIXEL QUEST',        tag: 'UI/UX',      image: 'https://picsum.photos/seed/kai6/400/300' },
];
...
export function About({ onNavigate: _nav }: AboutProps) {
  ...
  return (
    <div ref={ref} className="relative w-full min-h-screen overflow-y-auto overflow-x-hidden"
      style={{ background: '#080608', color: '#e8e0e3' }}>

      {/* Pixel grid bg */}
      <div className="pointer-events-none absolute inset-0 opacity-10" style={{
        backgroundImage: 'linear-gradient(rgba(204,17,51,0.08) 1px,transparent 1px),linear-gradient(90deg,rgba(204,17,51,0.08) 1px,transparent 1px)',
      ...
      <div className="relative z-10 px-6 lg:px-10 xl:px-14 py-8 max-w-[1400px] mx-auto space-y-6">
        ...
        <motion.div
          ...
          style={{ gridTemplateColumns: 'minmax(220px,280px) 1fr minmax(240px,320px) minmax(240px,300px)' }}
        >
...
function ProfileCard({ inView }: { inView: boolean }) {
  ...
  return (
    <HudPanel className="flex flex-col items-center gap-3 py-5 px-4">
      {/* Avatar frame */}
      <motion.div
        animate={{ y: [0, -6, 0] }}
      ...
      >
        <div className="relative w-28 h-28 flex items-center justify-center"
          style={{
            ...
            boxShadow: glitch
              ? `-3px 0 0 rgba(0,140,255,0.5), 3px 0 0 rgba(255,34,68,0.5), 0 0 20px ${RED}55`
              : `0 0 20px ${RED}33, inset 0 0 20px rgba(0,0,0,0.6)`,
            transition: 'box-shadow 0.05s',
          }}
        >
          <img src="/assets/icon-kai-cat3.png" alt="Kai Shi Avatar"
            className="w-20 h-20 object-contain"
            style={{ ...PIX, filter: `drop-shadow(0 0 8px ${RED})`, transform: glitch ? 'translateX(2px)' : 'none', transition: 'transform 0.05s' }}
          />
          {/* Corner decorators */}
      ...
      <div className="text-center">
        <div className="font-pixel text-xl text-white tracking-widest"
          style={{
            textShadow: glitch ? `3px 0 #ff0044,-3px 0 #0088ff,0 0 20px ${RED}` : `0 0 15px ${RED}66`,
      ...
      <div className="w-full space-y-2 px-1">
        ...
        >
          ...
          <SegmentedBar value={75} color={RED} />
        </motion.div>
...
function BiographyPanel({ inView }: { inView: boolean }) {
  return (
    <HudPanel className="flex flex-col gap-4 py-5 px-5">
      ...
      >
        <p>
          A digital traveler building immersive worlds through code, creativity, and endless nights.
        </p>
        <p>
          I turn ideas into interactive experiences and bring anime vibes to the web.
        </p>
      </motion.div>
      ...
      <motion.div
        ...
        style={{
          ...
          borderRight: '1px solid rgba(204,17,51,0.2)',
          borderBottom: '1px solid rgba(204,17,51,0.2)',
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              <img src={stat.icon} alt="" className="w-4 h-4 object-contain shrink-0"
                style={{ ...PIX, filter: `drop-shadow(0 0 3px ${RED}88)` }}
              />
              <span className="font-pixel text-[8px] flex-1 tracking-wide" style={{ color: '#a09098' }}>
                {stat.label}
              </span>
              <span className="font-pixel text-[9px]" style={{ color: RED }}>{stat.value}</span>
            </div>
...
function SkillTreePanel({ inView }: { inView: boolean }) {
  ...
  return (
    <HudPanel className="flex flex-col gap-3 py-5 px-5">
        ...
        {SKILLS.map((skill, i) => (
          ...
          >
            <motion.div
              ...
              animate={hovered === i ? { boxShadow: `0 0 16px ${RED}88` } : { boxShadow: `0 0 6px ${RED}22` }}
              style={{
                border: skill.locked ? '1px solid rgba(61,15,26,0.5)' : `1px solid ${RED}55`,
                background: skill.locked ? 'rgba(10,4,8,0.8)' : 'rgba(20,4,12,0.9)',
            ...
            >
              {skill.locked ? (
                ...
                <motion.img
                  ...
                  style={PIX}
                  animate={hovered === i
                    ? { filter: `drop-shadow(0 0 8px ${RED}) brightness(1.3)` }
                    : { filter: `drop-shadow(0 0 3px ${RED}88) brightness(1)` }
                  }
            ...
            <span className="font-pixel text-[7px] tracking-wider text-center"
              style={{ color: skill.locked ? '#3d1a22' : '#a09098' }}>
              {skill.label}
            </span>
            {/* Connector lines between rows */}
            {i < 3 && (
              <div className="absolute mt-[68px] w-px h-3" style={{ background: `${RED}33` }} />
            )}
          </motion.div>
        ))}
      </div>

      {/* Connector lines visually connecting rows */}
      ...
      <button
        className="w-full font-pixel text-[8px] py-2 tracking-widest transition-all group"
        style={{
        ...
        onMouseEnter={e => {
          ...
          (e.currentTarget as HTMLElement).style.boxShadow = `0 0 12px ${RED}44`;
        }}
        onMouseLeave={e => {
      ...
      <HudPanel className="p-0 overflow-hidden">
        <div className="grid" style={{ gridTemplateColumns: '260px 1fr' }}>
          ...
          <div className="px-5 py-5 flex flex-col gap-4 border-r"
            ...
            >
              <h2 className="font-pixel text-base text-white leading-tight mb-2"
                style={{ textShadow: `0 0 12px ${RED}44` }}>
                MY DIGITAL<br />LOGS
              </h2>
                ...
                {GALLERY_FILTERS.map(f => (
                  <button key={f} onClick={() => setFilter(f)}
                    ...
                    style={{
                      ...
                      boxShadow: filter === f ? `0 0 10px ${RED}55` : 'none',
                    }}
                  >
                    {f}
                  </button>
                  ...
                  <button key={i} onClick={() => setSelected(i)}>
                    <motion.span
                      ...
                      animate={{
                        ...
                        rotate: 45,
                        boxShadow: i === selected ? `0 0 8px ${RED}` : 'none',
                      }}
                      transition={{ duration: 0.2 }}
                    />
                  </button>
          ...
          <div className="relative py-5 px-5 overflow-hidden">
            <div className="flex gap-3 items-stretch h-full" style={{ minHeight: '260px' }}>
                ...
                {filtered.map((item, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      style={{
                        border: isSelected ? `1.5px solid ${RED}` : '1px solid rgba(61,15,26,0.6)',
                        boxShadow: isSelected ? `0 0 20px ${RED}55` : 'none',
                        minWidth: 0,
                    ...
                    >
                      ...
                      <img src={item.image} alt={item.title}
                        ...
                        style={{
                          filter: isSelected ? 'brightness(0.85) saturate(1.2)' : 'brightness(0.45) saturate(0.7)',
                      ...
                      {isSelected && (
                        <div className="absolute inset-0 pointer-events-none"
                          style={{ background: 'linear-gradient(to top, rgba(4,1,10,0.88) 0%, transparent 55%)' }} />
                      )}
                      ...
                      <AnimatePresence>
                        {isSelected && (
                          ...
                          >
                            <div className="font-pixel text-[9px] text-white tracking-wider">{item.title}</div>
...
function SystemStatusBar() {
  ...
  return (
    <div className="flex items-center justify-between px-3 py-2"
      style={{ border: `1px solid rgba(61,15,26,0.6)`, background: 'rgba(8,4,6,0.8)' }}>
      <div className="flex items-center gap-2">
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: `0 0 20px rgba(204,17,51,0.06), inset 0 0 20px rgba(0,0,0,0.4)`,
        backdropFilter: 'blur(4px)',
      }}
    >
      {/* Subtle top border glow */}
      <div className="absolute top-0 left-0 right-0 h-px pointer-events-none"
        style={{ background: `linear-gradient(90deg, transparent, ${RED}44, transparent)` }} />
      {children}
    </div>
  );
}
...
/* Segmented progress bar — matches the reference style */
...
function TinyCorner({
  ...
  pos: string; flip?: string; size?: number;
}) {
  ...
  return <div className={pos.includes('top') ? (pos.includes('left') ? 'top-0 left-0' : 'top-0 right-0') : (pos.includes('left') ? 'bottom-0 left-0' : 'bottom-0 right-0')}
...
function NavArrow({ dir, onClick }: { dir: 'left'|'right'; onClick: () => void }) {
  return (
    <motion.button
      onClick={onClick}
      whileHover={{ scale: 1.1, boxShadow: `0 0 14px ${RED}66` }}
      whileTap={{ scale: 0.92 }}
